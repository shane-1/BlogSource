...
已经用了ChatGPT一段时间了，我一开始对其态度就是怀疑和审慎，到现在少了分怀疑，更多了分审慎。其和其代表的超大规模语言模型，对于先今以网络空间主导的社会形态影响都是长远而深刻的，我个人认为其未来走向会直接影响人类发展趋势（如此也能理解很多人第三次工业革命的论调）。
这样分析也许太过宏达叙事，我只从程序员角度分析：超大规模语言模型第一波及的必然是以语言为核心的工作，而其影响必然从简单到复杂。目前ChatGPT在编程主要以写代码片段为主，没有大型软件架构能力。但现在如此，未来一直如此吗？语言和逻辑本身就是此类模型擅长的核心，也许在目前的已经瓶颈靠堆砌的半导体计算机硬件跑不出，但量子计算机、生物计算机等可能的下一代计算平台呢？更大尺度看如果其能够自行迭代呢？其也具有逆熵的生命欲望呢？如果只是把ChatGPT为代表的以类神经网络计算模型之后的发展理解为对于文字推理的数字计算，那人类的智慧是否也会被简化到神经元放电。
也许其看起来很简陋，但在数量级显著增加后表现出一些人类难以理解的能力（如ChatGPT撰写论文的能力），也许下一个人类文明的奇点时刻真会发生在类神经网络计算模型。无论如何，资本已经在路上，剩下的与诸位且走且看。
